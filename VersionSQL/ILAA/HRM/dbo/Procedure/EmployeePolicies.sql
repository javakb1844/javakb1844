/****** Object:  Procedure [dbo].[EmployeePolicies]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[EmployeePolicies] 
...
BEGIN
		 ...
		 BEGIN
		  if(@IsEmployeeOnly=0)
			  ...
			  select LookPolicy.*,HRPolicy.PolicyValue,EmployeePolicy.EmployeePolicyId,EmployeePolicy.PolicyValue as EmployeePolicyValue from LookPolicy 
				...
				where HRPolicy.GroupTypeId=1 and LookPolicy.LookPolicyGroupId=@LookPolicyGroupId  and HRPolicy.LookDesignationId=@LookDesignationId 
			   END
           else
			 BEGIN
		  --select LookPolicy.*,HRPolicy.PolicyValue,EmployeePolicy.EmployeePolicyId,EmployeePolicy.PolicyValue as EmployeePolicyValue from LookPolicy 
    ...
    --        where LookPolicy.LookPolicyGroupId=@LookPolicyGroupId  and HRPolicy.LookDesignationId=@LookDesignationId  and EmployeePolicy.EmployeeId =@EmployeeId
		    ...
		    select EmployeePolicy.LookPolicyId,EmployeePolicy.EmployeePolicyId,EmployeePolicy.PolicyName,EmployeePolicy.PolicyUnit,
			 ...
			 inner join HRPolicy on Employee.LookDesignationId=HRPolicy.LookDesignationId and EmployeePolicy.LookPolicyId=HRPolicy.LookPolicyId
             where  HRPolicy.GroupTypeId=1  and (Employee.IsUserOnly=0  or Employee.IsUserOnly is null) and  EmployeePolicy.LookPolicyGroupId=@LookPolicyGroupId  and EmployeePolicy.EmployeeId =@EmployeeId ORDER BY LookPolicyId
		...
		END
		 ...
		 BEGIN
			...
			BEGIN
			 ...
			 inner join HRPolicy on HRPolicy.LookPolicyId=LookPolicy.LookPolicyId
			 left join EmployeePolicy on EmployeePolicy.LookPolicyId= LookPolicy.LookPolicyId and EmployeePolicy.EmployeeId =@EmployeeId
            where HRPolicy.GroupTypeId=1 and  HRPolicy.LookDesignationId=@LookDesignationId 
		   ...
		   BEGIN
		    ...
		    select EmployeePolicy.LookPolicyId,EmployeePolicy.PolicyName,EmployeePolicy.PolicyUnit,
			 ...
			 inner join employee on Employee.EmployeeId=EmployeePolicy.EmployeeId
			 inner join HRPolicy on Employee.LookDesignationId=HRPolicy.LookDesignationId and EmployeePolicy.LookPolicyId=HRPolicy.LookPolicyId
             where HRPolicy.GroupTypeId=1 and (Employee.IsUserOnly=0  or Employee.IsUserOnly is null) and  EmployeePolicy.EmployeeId =@EmployeeId ORDER BY LookPolicyId
